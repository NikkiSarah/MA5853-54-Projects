...
def save_created_objects2():
    object_names = [vocabulary, unknown_words, unknown_word_counts, vodafone_reviews,
                    coarse_pos_df, fine_pos_df, ent_text_df, ent_label_df]
    ...
    coarse_pos_df.to_csv('part2_coarse_pos_df.csv', header=True, index=True)
    fine_pos_df.to_csv('part2_fine_pos_df.csv', header=True, index=True)
    ent_text_df.to_csv('part2_ent_text_df.csv', header=True, index=True)
    ent_label_df.to_csv('part2_ent_label_df.csv', header=True, index=True)
...
clean_polarity_stats = sentiment_df.groupby('nps_category')['clean_polarity'].agg(
    [np.mean, np.std, np.min, np.max, np.median])

subjectivity_stats = sentiment_df.groupby('score')['text_subjectivity'].agg([np.mean, np.std, np.min, np.max, np.median])
clean_subjectivity_stats = sentiment_df.groupby('nps_category')['clean_subjectivity'].agg(
    [np.mean, np.std, np.min, np.max, np.median])
...
def save_created_objects3():
    object_names = [review_corpus, promoter_corpus, passive_corpus, detractor_corpus,
                    ...
                    vodafone_reviews,
                    sentiment_df, polarity_stats, clean_polarity_stats, subjectivity_stats, clean_subjectivity_stats,
                    rating_cm, nps_cm]
    ...
    clean_polarity_stats.to_csv('part3_clean_polarity_stats', header=True)
    rating_cm.to_csv('part3_rating_cm', header=True)
    nps_cm.to_csv('part3_nps_cm', header=True, index=True)
